# -*- coding: utf-8 -*-
"""
校准报告智能重命名与多层级自动归档工具 v24.0 (极速双引擎增强版)
======================================================================
核心特性：
1. 【多模式扫描与遍历】
   - 支持递归深度扫描指定文件夹，自动排除已归档文件夹防重复。
   - 自动检测并解压 ZIP 压缩包（自动纠正 Windows GBK/UTF-8 压缩包文件名乱码）。
   - 支持命令行传参、拖拽文件夹输入与交互式运行。
   - 支持 --dry-run 试运行预览模式与 --copy 复制模式。
2. 【PyMuPDF + RapidOCR 智能双引擎】
   - 默认 PyMuPDF 毫秒级极速解析。
   - 智能乱码层与扫描图片检测，自动无缝切换 RapidOCR 高精度识别。
3. 【全景台账天眼词库】
   - 聚合扫描快检设备清单与定量量值溯源总表全部子表。
   - 建立编号、出厂号、设备名、组别、实验室等多重双向检索索引。
   - 智能识别【快检设备】与【定量设备】，自动补全缺失信息。
4. 【多机构规则引擎】
   - 覆盖东莞帝恩 (DN)、深圳达丰 (DAF)、广州中广测 (NEM)、深圳计量院 (SMQ)、中检深圳 (CCIC) 及通用第三方。
5. 【多层级智能结构归档】
   - 快检设备：【归档完成】校准证书库/快检设备/{YYYY}年{MM}月{项目组}设备校准证书/
   - 定量设备：【归档完成】校准证书库/定量设备/{YYYY}年检定校准证书/{YYYY}年{MM}月{机构}校准证书/{YYYY}年{MM}月{组别}校准证书-{机构}/
   - 未分类设备：【归档完成】校准证书库/未分类设备/{YYYY}年{MM}月校准证书/
6. 【多维度报表生成】
   - 自动生成格式美化的高清 Excel 汇总台账（自动列宽与商务配色）与 TXT 处理统计清单。
======================================================================
"""
...
class LedgerDatabase:
    ...
    def __init__(self):
        ...
        self.all_known_assets = set() # 所有有效设备编号集合 (按长度倒序排序)
        self.sorted_asset_list = []

    def load(self, base_dir='.'):
        print("\n" + "=" * 70)
        print("【步骤 1/4】加载仪器设备台账天眼词库...")
        print("=" * 70)
    ...
    def _load_quantitative_excel(self, fpath):
        print(f"📘 正在加载定量量值溯源台账：{os.path.basename(fpath)}")
        try:
            xl = pd.ExcelFile(fpath)
            for sheet in xl.sheet_names:
...
def build_new_filename(issuer, fields):
    ...
    if issuer == 'DN':
        parts = [
            fields.get('lab_name', '未知实验室'),
            fields.get('asset_no', '未知编号'),
            fields.get('cal_date', '未知校准'),
            fields.get('issue_date', fields.get('cal_date', '未知签发')),
            fields.get('inst_name', '未查找到')
        ]
    elif issuer == 'NEM':
        code = fields.get('serial_no') or fields.get('asset_no') or '未知编号'
        parts = [
            code,
            fields.get('inst_name', '未查找到'),
            fields.get('cal_date', '未知校准'),
            fields.get('group', '未知组别')
        ]
    else:
        code = fields.get('asset_no') or fields.get('serial_no') or '未知编号'
        parts = [
            code,
            fields.get('inst_name', '未查找到'),
            fields.get('cal_date', '未知校准'),
...
def build_archive_dir(archive_root, issuer, fields):
    ...
    if device_type == '快检':
        ...
        if not lab_name.endswith('项目组') and not lab_name.endswith('街道') and not lab_name.endswith('组'):
            ...
            folder_tag = lab_name
        month_folder = f"{year}年{month}月{folder_tag}设备校准证书"
        ...
        return os.path.join(archive_root, '定量设备', year_folder, month_issuer_folder, detail_folder)

    else:
        # 未分类设备
        return os.path.join(archive_root, '未分类设备', f"{year}年{month}月校准证书")
...
def run_archive_process(target_dir='.', archive_root='./【归档完成】校准证书库',
    ...
    for idx, pdf_path in enumerate(pdf_list, 1):
        ...
        # 构建新文件名与目标路径
        new_stem = build_new_filename(issuer, fields)
        ...
        while os.path.exists(target_file_path) and os.path.abspath(target_file_path) != os.path.abspath(pdf_path):
            new_filename = f"{original_new_stem}_{conflict_suffix}.pdf"
            target_file_path = os.path.join(target_dir_path, new_filename)
            conflict_suffix += 1
